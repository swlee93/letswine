...
const AlcoholSafetyBottom = lazy(() => import('src/features/form/alcoholSafetyBottom'));

const Form: FC<{ selectedType: AVAILABLE_MENU_LIST | null }> = ({ selectedType }) => {
        ...
        <Grid container>
          <Grid item xs={12}>
            <Grid container xs={12}>
              <Grid item xs={6}>
                {selectedType && <Typography variant='h6'>{mapMenuValueToMenuLabel(selectedType)}</Typography>}
              </Grid>
            </Grid>
          </Grid>
          <Divider />
          <Grid item xs={12}>
            <Suspense fallback={() => <div>Loading...</div>}>
              ...
              {selectedType === AVAILABLE_MENU_LIST.UNTITLED_1 && <Untitled1 order={1} />}
              {selectedType === AVAILABLE_MENU_LIST.UNTITLED_2 && <Untitled2 order={2} />}
              {selectedType === AVAILABLE_MENU_LIST.UNTITLED_3 && <Untitled3 order={3} />}
              {selectedType === AVAILABLE_MENU_LIST.SCENT && <Scent />}
              {selectedType === AVAILABLE_MENU_LIST.VARIETIES && <Varieties />}
              {selectedType === AVAILABLE_MENU_LIST.DRINKING_GUIDE && <DrinkingGuide />}
              {selectedType === AVAILABLE_MENU_LIST.INDICATION && <Indication />}
              {selectedType === AVAILABLE_MENU_LIST.BRAND_SHOP_TOP && <BrandShopTop />}
              {selectedType === AVAILABLE_MENU_LIST.BRAND_SHOP_BOTTOM && <BrandShopBottom />}
              {selectedType === AVAILABLE_MENU_LIST.ALCOHOL_SAFETY_BOTTOM && <AlcoholSafetyBottom />}
            </Suspense>
          </Grid>
        </Grid>
      </Container>
